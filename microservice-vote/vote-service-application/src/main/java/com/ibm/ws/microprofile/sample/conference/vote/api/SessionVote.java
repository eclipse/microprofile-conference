...
public class SessionVote {
    ...
    private final Logger log = Logger.getLogger(SessionVote.class.getName());

    private final AtomicLong nextAttendeeId = new AtomicLong(0);
    private final Map<Long, Attendee> attendees = new HashMap<Long, Attendee>();

    private final AtomicLong nextSessionId = new AtomicLong(0);
    private final Map<Long, SessionRating> allRatings = new HashMap<Long, SessionRating>();

    private final Map<String, List<Long>> ratingIdsBySession = new HashMap<String, List<Long>>();

    private final Map<Long, List<Long>> ratingIdsByAttendee = new HashMap<Long, List<Long>>();
    ...
    public List<SessionRating> allSessionVotes(String sessionId) {
        log.log(Level.INFO, "> allSessionVotes() " + sessionId);
        for (Map.Entry<String, List<Long>> entry : ratingIdsBySession.entrySet()) {
            log.log(Level.INFO, entry.getKey() + " = " + entry.getValue());
        }
