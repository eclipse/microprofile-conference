...
The Schedule service is provided using a special microprofile release of Payara Micro link:http://www.payara.fish/payara_micro[Payara Micro] which can be downloaded from link:http://search.maven.org/#search%7Cga%7C1%7Ca%3A%22payara-microprofile%22[Maven Central]
...
*Port Bindings*
By default the microservice will bind to port 8080, if this is not available it will bind to the next available port e.g. 8081. To specifiy the port use --port as a command line option
